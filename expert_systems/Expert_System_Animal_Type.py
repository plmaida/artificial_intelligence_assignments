...
if (cell_q == "yes"):   #if the animal is NOT multi-celled there is no reason to continue with the questions. 
    bones_q = input("Does it have bones? (Yes or No) ").lower()
    if (bones_q == "yes"): #if the animal does NOT have bones there is no reason to continue with the questions.
        gills_q = input("Does it Gills? (Yes or No) ").lower()
        if (gills_q == "no"): #if the animal HAS gills there is no reason to continue with the questions.
            cover_q = input("Does it have fur or feathers? ").lower()
            wings_q = input("Does it have wings? (Yes or No) ").lower() 

system.declare(Cell(cell= T_F(cell_q)), Bones(bones= T_F(bones_q)), 
               Gills(gills = T_F(gills_q)), Cover(cover= cover_q), Wings(wings= T_F(wings_q)))
system.run()
