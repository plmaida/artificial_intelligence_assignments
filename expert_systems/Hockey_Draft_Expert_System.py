...
position_q = input(
    "What position does this player play? (Forward, Defense or Goalie) "
).lower()
if position_q == "goalie":
    print(
        "This expert system does not rank goalies due to their unpredictability! Pick a different player"
    )
# elif(position_q == "forward" or position_q == "defense"):
    ...
    system.declare(
        Position(positions=position_q),
        Hockey_IQ(hockey_iq=hockey_iq_q),
        Skating(skating=skating_q),
        LineMate(linemate=T_F(linemate_q)),
        Off_driver(off_driver=off_driver_q),
        Def_aware(def_aware=def_aware_q),
    )
    system.run()
# else:
